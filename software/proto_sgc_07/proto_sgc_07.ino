...
void setup()
{
    ...
    if (!Konnekting.isFactorySetting())
    {
		...
		param_close_time_contact = (unsigned int)Konnekting.getUINT8Param(PARAM_drivecurrent_autozero);
		param_close_time_nocontact = (unsigned int)Konnekting.getUINT8Param(PARAM_drivecurrent_autozero);
    ...
    Debug.println(F("inclosed changed: %d"), in_closed );
  	...
  	if(!in_closed)
  	{
  	  // detected opening action
  		if(param_close_ext ||	(glob_last_command_valid && (glob_last_command == COMOBJ_cmd_open || glob_last_command == COMOBJ_cmd_partly_open || glob_last_command == COMOBJ_cmd_position))) //// when there was no command (=external opening) and the parameter for closing on external command is set or when last command is valid and was open, openpart or position 
  		{
        if(param_close_time_contact)
    	  glob_AutoClose_Wait4Barrier = true;
        if(param_close_time_nocontact)
    	  glob_AutoClose_Cnt2Close = param_close_time_nocontact * 6; //(60000 / T5_CYCLETIME);
  	  }
  	}
    // End AutoClose
  }
